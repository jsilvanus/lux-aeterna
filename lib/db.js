...
export async function listMemories() {
  ...
  const pool = getPool();
  const result = await pool.query(`
    SELECT id, COALESCE(author, name) AS author, memory, memory_date, COALESCE(images, '[]'::jsonb) AS images, created_at
    FROM memories
...
export async function addMemory(author, memory, memoryDate, images = []) {
  ...
  const result = await pool.query(
    `
      ...
      RETURNING id, author, memory, memory_date, COALESCE(images, '[]'::jsonb) AS images, created_at;
    `,
