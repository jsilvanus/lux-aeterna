...
export async function listMemories() {
  ...
  const pool = getPool();
  const result = await pool.query(`
    SELECT id, COALESCE(author, name) AS author, memory, memory_date, created_at
    FROM memories
...
export async function addMemory(author, memory, memoryDate) {
  await initDb();
  const pool = getPool();
  const id = randomUUID();
  const result = await pool.query(
    `
      INSERT INTO memories (id, name, author, memory, memory_date)
      VALUES ($1, $2, $2, $3, $4)
      RETURNING id, author, memory, memory_date, created_at;
    `,
    [id, author, memory, memoryDate],
  );
